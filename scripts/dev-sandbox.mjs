...
const apiPort = Number(process.env.WORTHLANE_SANDBOX_API_PORT ?? 3301);
const desktopPort = Number(process.env.WORTHLANE_SANDBOX_DESKTOP_PORT ?? 3303);
assert([apiPort, desktopPort].every(port => Number.isInteger(port) && port >= 1024 && port <= 65535) && apiPort !== desktopPort, 'Use distinct unprivileged local ports.');
...
const env = { ...process.env, ...secrets, DATABASE_URL: db.href, NODE_ENV: 'development',
  PLAID_ENV: 'sandbox', WORTHLANE_API_URL: `http://127.0.0.1:${apiPort}/api`, NEXT_TELEMETRY_DISABLED: '1',
...
for (const [app, host, port] of [['api', '127.0.0.1', apiPort], ['desktop', 'localhost', desktopPort]]) {
  const cwd = resolve('apps', app);
  const require = createRequire(resolve(cwd, 'package.json'));
  const child = spawn(process.execPath, [require.resolve('next/dist/bin/next'), 'dev', '-H', host, '-p', String(port)], { cwd, env: { ...env, WORTHLANE_HTTP_DIST_DIR: `.next-http-${port}` }, stdio: 'inherit' });
  children.push(child);
  child.on('error', () => stop(1));
  child.on('exit', code => { if (!stopping) stop(code || 1); });
}
console.log(`Persistent local Sandbox: desktop http://localhost:${desktopPort}; mobile API http://localhost:${apiPort}/api.`);
