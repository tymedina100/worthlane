...
export async function revokePlaidData(item: { id: string; userId: string; itemId: string }, accountId?: string) {
  return prisma.$transaction(async db => {
    ...
    await removePlaidAccountData(db, item.userId, accounts.map(account => account.id));
...
/** Within the caller's transaction, preserve authored entries and purge imports/caches. */
export async function removePlaidAccountData(db: Prisma.TransactionClient, userId: string, ids: string[]) {
  if (!ids.length) return;
  const manualWhere = { userId, accountId: { in: ids }, plaidTransactionId: null, isManual: true };
  if (await db.transaction.count({ where: manualWhere })) {
    const manual = await db.account.create({ data: {
      userId, name: "Saved manual entries", source: "MANUAL", type: "OTHER", currentBalance: 0,
    } });
  ...
  const revokedTransactions = await db.transaction.findMany({ where: {
    userId, accountId: { in: ids }, plaidTransactionId: { not: null },
  ...
  await db.recurringTransaction.deleteMany({ where: { userId, OR: [
    { accountId: { in: ids } }, { normalizedMerchant: { in: merchants } },
  ] } });
  await db.account.deleteMany({ where: { userId, id: { in: ids } } });
  ...
  await db.netWorthSnapshot.deleteMany({ where: { userId } });
  await db.nudge.deleteMany({ where: { userId } });
}
