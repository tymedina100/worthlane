...
beforeEach(() => {
  vi.clearAllMocks(); mocks.save.mockImplementation(async (item, _provider, selected) => ({ accountMap: new Map(selected.map((a: { account_id: string }) => [a.account_id, "local-account"])), consentRevision: (item.consentRevision ?? 0) + 1 })); mocks.update.mockResolvedValue({ count: 1 });
...
describe("investment-only sync", () => {
  it("does not overwrite revocation with a late successful investment snapshot", async () => {
    ...
    expect(mocks.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: item.id, consentRevision: 4 } }));
  });
  it("persists balances without initializing transaction products, even on forced refresh", async () => {
    ...
    expect(mocks.save).toHaveBeenCalledWith(item, expect.any(Array), accounts);
  ...
  it("rejects an investment connection with no shared investment accounts", async () => {
    ...
    expect(mocks.save).toHaveBeenCalledWith(item, [], []); expect(mocks.sync).not.toHaveBeenCalled();
  });
