...
import { Prisma, prisma, NudgeType } from "@worthlane/db";
import {
...
export async function generateNudgesForUser(userId: string): Promise<void> {
  const createdCount = await prisma.$transaction(async db => {
    ...
    const impulseAgg = await db.transaction.aggregate({
      where: {
        isImpulse: true,
    ...
    const impulseFlaggedToday = await db.transaction.findFirst({
    ...
    for (const nudge of nudges) {
      const result = await db.nudge.createMany({ data: { userId, day, ...nudge }, skipDuplicates: true });
      count += result.count;
    }
    return count;
  }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable, timeout: 30_000 });
  // Only send the generic notification after the data transaction commits.
  if (createdCount > 0) await sendPushToUser(userId);
}
