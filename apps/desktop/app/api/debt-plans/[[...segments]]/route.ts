...
async function proxy(request: NextRequest, context: Context) {
  ...
  if (segments.length > 1 || (request.method === "POST" && segments.length) || (request.method === "PATCH" && segments.length !== 1)) return errorResponse("Plan route not found", 404, "NOT_FOUND");
  let body: unknown;
  if (request.method !== "GET") {
    const schema = request.method === "POST" ? debtPlanInputSchema : z.object({ revision: z.number().int().positive(), input: debtPlanInputSchema }).strict();
    const parsed = schema.safeParse(await request.json().catch(() => null));
    if (!parsed.success) return errorResponse("Check the plan details.", 400, "VALIDATION_ERROR");
    body = parsed.data;
  }
  try {
    const response = await authenticatedServerRequest(await cookies(), `/debt-plans${segments.length ? `/${encodeURIComponent(segments[0]!)}` : ""}`, { method: request.method, ...(body ? { body: JSON.stringify(body) } : {}) });
